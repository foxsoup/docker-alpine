...
RUN echo "expose_php=off" > /usr/local/etc/php/conf.d/99_custom.ini \
  ...
  && echo "date.timezone=Europe/London" >> /usr/local/etc/php/conf.d/99_custom.ini \
...
&& apk add --no-cache --virtual .phpize-deps $PHPIZE_DEPS zlib-dev icu-dev pcre-dev \
&& pecl install apcu redis xdebug \
&& docker-php-ext-enable apcu redis \
&& docker-php-ext-install intl bcmath pdo_mysql opcache zip sockets \
&& apk del .phpize-deps \
